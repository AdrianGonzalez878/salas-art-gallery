...
export const metadata: Metadata = {
  title: 'Promociones',
  description:
    'Obras con descuento en Salas Art Gallery. Descubre piezas de arte contemporáneo en promoción.',
  keywords: ['promociones arte', 'obras con descuento', 'Salas Art Gallery'],
  alternates: {
...
export default async function PromocionesPage({
  ...
  searchParams: Promise<{ page?: string; etiqueta?: string; orden?: string }>
}) {
  ...
  const etiquetas = uniqueEtiquetas(lista.flatMap((p) => p.etiquetas ?? []))
  ...
  if (etiquetaFiltro) {
    lista = lista.filter((p) =>
      (p.etiquetas ?? []).some((tag) => tag.toLowerCase() === etiquetaFiltro.toLowerCase())
    )
  }
  ...
  const tituloSeccion = etiquetaFiltro || 'Selección con precio especial'
  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-10 pb-16">
        {totalProductosOferta === 0 && !etiquetaFiltro ? (
          <AnimateInView className="flex flex-col items-center py-20 text-center">
            ...
            <h1 className="font-display text-2xl font-light text-gray-900 mb-3">No hay una selección especial activa</h1>
            ...
            <div>
                  ...
                  <div className="mt-6 pt-6 border-t border-gray-100">
                    ...
                    <div className="flex flex-wrap justify-center gap-2">
                      ...
                      {etiquetas.map((tag) => (
                        <Link
                          key={tag}
                          href={etiquetaHref(tag, '/promociones')}
                          className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-5">
                {productosPagina.map((producto, index) => {
                  ...
                  const tagsPreview = (producto.etiquetas ?? []).slice(0, 2).join(' · ')
