'use client'

import { useMemo, useState } from 'react'

const BRAND_NAME = 'Salas Art Gallery'

/** Mensaje completo para pegar (copiar, WhatsApp): más contexto que solo la URL */
function buildFullShareMessage(productTitle: string, productUrl: string): string {
  return `Te comparto esta obra de ${BRAND_NAME}:

"${productTitle}"

${productUrl}

Arte contemporáneo · Obras únicas`
}

/** Texto para X: la URL va en el parámetro aparte del intent */
function buildTwitterShareText(productTitle: string): string {
  return `"${productTitle}"\n\nDescúbrela en ${BRAND_NAME}`
}

interface ProductShareButtonsProps {
  title: string
  /** URL absoluta del producto (ej. https://tudominio.com/productos/slug). Se pasa desde el servidor para evitar hydration mismatch. */
  url: string
}

export default function ProductShareButtons({ title, url }: ProductShareButtonsProps) {
  const [copied, setCopied] = useState(false)

  const fullMessage = useMemo(() => buildFullShareMessage(title, url), [title, url])

  const shareLinks = useMemo(
    () => ({
      whatsapp: `https://wa.me/?text=${encodeURIComponent(fullMessage)}`,
      facebook: `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(url)}`,
      twitter: `https://twitter.com/intent/tweet?url=${encodeURIComponent(url)}&text=${encodeURIComponent(buildTwitterShareText(title))}`,
    }),
    [fullMessage, title, url]
  )

  const handleCopyShareText = async () => {
    try {
      await navigator.clipboard.writeText(fullMessage)
      setCopied(true)
      setTimeout(() => setCopied(false), 2000)
    } catch {
      // fallback
    }
  }

  const btnBase =
    'flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-full cursor-pointer transition-all shadow-sm hover:scale-105 hover:shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2'

  return (
    <div className="mt-3 pt-3 border-t border-gray-200 sm:mt-6 sm:pt-6">
      <div className="mb-2 sm:mb-3">
        <p className="text-xs sm:text-sm font-semibold text-gray-700">Compartir</p>

      </div>
      <div className="flex flex-wrap gap-2">
        <a
          href={shareLinks.whatsapp}
          target="_blank"
          rel="noopener noreferrer"
          className={`${btnBase} bg-[#25D366] text-white hover:bg-[#20bd5a] focus-visible:ring-[#25D366]`}
          aria-label="Compartir en WhatsApp"
        >
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413Z" />
          </svg>
        </a>
        <a
          href={shareLinks.facebook}
          target="_blank"
          rel="noopener noreferrer"
          className={`${btnBase} bg-[#1877F2] text-white hover:bg-[#166fe5] focus-visible:ring-[#1877F2]`}
          aria-label="Compartir en Facebook"
        >
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
          </svg>
        </a>
        <a
          href={shareLinks.twitter}
          target="_blank"
          rel="noopener noreferrer"
          className={`${btnBase} bg-gray-900 text-white hover:bg-black focus-visible:ring-gray-900`}
          aria-label="Compartir en X (Twitter)"
        >
          <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
            <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
          </svg>
        </a>
        <button
          type="button"
          onClick={handleCopyShareText}
          className={
            copied
              ? `${btnBase} bg-green-600 text-white hover:bg-green-600 focus-visible:ring-green-600`
              : `${btnBase} bg-amber-500 text-gray-900 hover:bg-amber-400 focus-visible:ring-amber-500`
          }
          aria-label={copied ? 'Mensaje copiado' : 'Copiar mensaje para compartir'}
          title={copied ? '¡Copiado!' : 'Copia un mensaje con el enlace listo para pegar'}
        >
          {copied ? (
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
            </svg>
          ) : (
            <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1" />
            </svg>
          )}
        </button>
      </div>
    </div>
  )
}
